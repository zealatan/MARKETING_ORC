...
def render_invest_simulation_tab(
    ...
    """Combined view: price/trigger chart on top, reinvest timeline below."""
    if reinvest is None:
        st.info("투자 시뮬레이션 결과를 계산할 수 없습니다.")
        return
    ...
    with col_a:
        show_zigzag = st.checkbox(
            "Show ZigZag H/L markers",
            value=True,
            key=f"sim_show_zigzag_{config.key}",
        )
    with col_b:
        show_buys = st.checkbox(
            "Show Trigger Buy markers",
            value=True,
            key=f"sim_show_buys_{config.key}",
        )

    price_fig = price_chart(
        inp, config, result,
        show_zigzag=show_zigzag, show_buys=show_buys,
    )
    price_fig.update_layout(height=430)
    st.plotly_chart(price_fig, use_container_width=True)

    if reinvest.timeline_df.empty:
        st.warning("배당 재투자 타임라인 데이터가 없습니다.")
        return

    timeline_fig = reinvest_timeline_chart(inp, config, reinvest.timeline_df)
    timeline_fig.update_layout(height=430)
...
def render_dividend_reinvest_tab(
    ...
    if not reinvest.timeline_df.empty:
        st.plotly_chart(
            reinvest_timeline_chart(inp, config, reinvest.timeline_df),
            use_container_width=True,
        )
        st.plotly_chart(
            share_count_chart(config, reinvest.timeline_df),
            use_container_width=True,
        )
