...
def render_invest_simulation_tab(
    ...
    """Portfolio-growth view: compare reinvest vs no-reinvest in one chart."""
    st.subheader("투자 결과 비교")
    st.caption("배당 재투자 여부에 따른 포트폴리오 가치 변화를 비교합니다.")
    ...
    if no_reinvest is None or no_reinvest.timeline_df.empty:
        fig = reinvest_timeline_chart(inp, config, reinvest.timeline_df)
        fig.update_layout(height=660)
        st.plotly_chart(fig, use_container_width=True)
        return

    fig = investment_comparison_chart(
        inp, config, reinvest.timeline_df, no_reinvest.timeline_df,
    )
    st.plotly_chart(fig, use_container_width=True)
