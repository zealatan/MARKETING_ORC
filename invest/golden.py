...
[data-testid="stMetric"] {{
    ...
    border: 1px solid rgba(0,18,25,0.10);
    border-radius: 18px;
    padding: 0.8rem 0.9rem;
    box-shadow: 0 8px 20px rgba(0,18,25,0.06);
}}
...
@media (max-width: 768px) {{
    ...
    .dashboard-card {{
        ...
        box-shadow: 0 10px 24px rgba(0,18,25,0.08);
    }}
    .dashboard-title {{
        font-size: 25px;
        line-height: 1.12;
    ...
    .cycle-card {{
        ...
        box-shadow: 0 8px 22px rgba(0,18,25,0.07);
    }}
    .cycle-title {{
        font-size: 20px;
        line-height: 1.2;
    ...
    table {{
        ...
        font-size: 12px !important;
        border-radius: 12px;
        box-shadow: 0 6px 16px rgba(0,18,25,0.06);
    }}
    th, td {{
        font-size: 12px !important;
        padding: 8px 8px;
...
def plot_dividend_charts_one_row(reinvest_df, ticker):
    """Render three dividend charts with compact mobile-friendly layout."""
    if reinvest_df.empty:
        return
    ...
    st.markdown(
        '<div class="cycle-card"><div class="cycle-title">배당 그래프 요약</div>'
        '<div class="cycle-subtitle">데스크톱에서는 3개 그래프가 한 줄에, 모바일에서는 화면 폭에 맞게 보기 좋게 표시됩니다.</div></div>',
        unsafe_allow_html=True,
    )

    def compact_layout(fig, title, y_title):
        fig.update_layout(
            title=dict(
                text=title,
            ...
            margin=dict(l=38, r=12, t=70, b=48),
            legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="left", x=0),
        )
        fig.update_xaxes(tickfont=dict(size=10), title=dict(font=dict(size=11)), automargin=True)
        fig.update_yaxes(tickfont=dict(size=10), title=dict(font=dict(size=11)), automargin=True)
        return fig
    ...
    with c1:
        ...
        fig_dps = compact_layout(fig_dps, f"{ticker}<br>Annual Dividend / Share", "Per Share")
        st.plotly_chart(fig_dps, width='stretch', config={"displayModeBar": False})
    ...
    with c2:
        ...
        fig_annual.add_trace(
            go.Bar(
                x=chart_df["연도"],
                y=chart_df["연간 배당금"],
                name="Annual Received",
            )
        )
        fig_annual = compact_layout(fig_annual, f"{ticker}<br>Annual Dividend Received", "Cash")
    ...
    with c3:
        ...
        fig_cum = compact_layout(fig_cum, f"{ticker}<br>Cumulative Dividend Received", "Cumulative Cash")
