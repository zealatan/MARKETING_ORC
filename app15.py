...
def build_drawdown_cycles(close, name, ticker, universe, threshold=0.10):
    ...
    for high_idx, high_price in highs:
        next_lows = [(low_idx, low_price) for low_idx, low_price in lows if low_idx > high_idx]
        if not next_lows:
            continue
    ...
    if mode == "시작일 일시불 투자":
        ...
        for d, price in close.items():
            ...
            # 이미 한 번 매수한 사이클에서는 다시 매수하지 않음
                ...
                # 전저점 이후 trigger% 이상 반등하면 새로운 전고점 사이클 시작
...
def render_drop_tables(status_df):
    ...
    sections = [
        ("10~20% 하락 구간", status_df[(status_df["변화율 숫자"] <= -10) & (status_df["변화율 숫자"] > -20)]),
        ("20~30% 하락 구간", status_df[(status_df["변화율 숫자"] <= -20) & (status_df["변화율 숫자"] > -30)]),
        ("30% 이상 하락 구간", status_df[status_df["변화율 숫자"] <= -30]),
    ]
...
def render_backtest_dashboard(start_date, end_date, threshold):
    st.markdown(
        """
<div class="cycle-card">
...
""",
        ...
        st.caption(
            f'단순 가격 상승률: '
            f'{summary["단순 가격 상승률"]:.2f}% / '
            f'총 보유 수량: '
            f'{summary["총 보유 수량"]:,.6f} / '
            f'최종 가격: '
            f'{summary["최종 가격"]:,.2f}'
        )
        ...
        # =====================================================
        # 핵심 수정 부분
        # trigger_drop_pct 기준으로 H/L 생성
        # =====================================================

        if show_backtest_high_low:

            dynamic_threshold = trigger_drop_pct / 100.0
        ...
        fig_price.update_layout(
            title=dict(
                text=(
                    f"{ticker} Backtest "
                    f"(Trigger={trigger_drop_pct}%)"
                ),
                font=dict(
                    size=FONT_CHART_TITLE,
                    color=COLOR_NAVY,
                ),
            ),
            xaxis_title="Date",
...
<div class="dashboard-card">
    ...
    <div class="dashboard-subtitle">
        ...
        현재 기준 전고점 대비 10% 이상 하락한 종목을 추적하고, 선택 종목 백테스트를 수행합니다.
    </div>
</div>
""",
    unsafe_allow_html=True,
...
tab_us, tab_us_etf, tab_kr, tab_kr_etf, tab_kr_cc, tab_global, tab_chart, tab_backtest = st.tabs(
    [
        "🇺🇸 미국 Top 100",
        "🇺🇸 미국 ETF Top 50",
        "🇰🇷 한국 Top 30",
...
with tab_backtest:
    render_backtest_dashboard(start_date, end_date, threshold)
